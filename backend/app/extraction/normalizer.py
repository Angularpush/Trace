...
def normalize_decimal(val: Any) -> Decimal:
    ...
    Strips currency symbols (₹, Rs., $, INR), commas, whitespace while preserving decimal points.
    """
    if val is None:
        return Decimal("0.00")
    ...
    # Remove currency symbols, INR/Rs text, commas, but keep digits, '-' and '.'
    val_str = re.sub(r"(?i)\b(inr|rs|usd|eur|gbp)\b", "", val_str)
    val_str = re.sub(r"[₹\$,\s]", "", val_str)
    # Remove any trailing / leading non-digit characters except '-' and '.'
...
def normalize_date(date_str: Optional[str]) -> Optional[str]:
    """
    Normalizes diverse date formats (DD-MM-YYYY, DD/MM/YYYY, 15th Aug 2024, ISO) to YYYY-MM-DD.
    """
