...
Extracts Invoice No, PO Ref, Supplier/Buyer details, GSTIN, Multi-row Line Items, Subtotal, Taxes, and Grand Total.
"""

import re
from decimal import Decimal
from typing import Dict, Any, List
from app.extraction.normalizer import normalize_decimal, normalize_date, clean_entity_name, clean_item_description
...
class InvoiceParser:
    ...
    def parse(extracted: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if supp_m and not any(k in supp_m.group(1).lower() for k in ["buyer", "sample", "test", "actual", "synthetic", "bill to"]):
            ...
            seller_match = re.search(r"(?:(?:Seller|Supplier\s*Name|From|Supplier)[:\s]+)([^\n,]+(?:Pvt|Ltd|Limited|Corp|Enterprises|Supplies|Fasteners|Systems|Solutions|Division|Technologies)?)", text, re.IGNORECASE)
            if seller_match and not any(k in seller_match.group(1).lower() for k in ["sample", "test", "actual", "synthetic", "buyer", "bill to"]):
        ...
        if row_starts:
            for r_idx, (item_no, line_idx) in enumerate(row_starts):
                ...
                clean_chunk = []
                for c in chunk:
                    if any(k in c.lower() for k in ["subtotal", "grand total", "taxable amount", "taxable subtotal", "invoice total", "terms:", "purpose:", "declaration"]):
                        break
